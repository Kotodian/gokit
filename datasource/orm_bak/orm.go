...
// InstanceID 实例ID
func (m Base) InstanceID() string {
	if m.instanceID == "" {
		m.instanceID = db.NewScope(m).InstanceID()
	}
	return m.instanceID
}
...
func (m *Base) AfterSave(scope *gorm.Scope) error {
	if funcs, ok := scope.Get("gorm:tx:objs"); ok {
		funcs.(map[string]*gorm.Scope)[m.InstanceID()] = scope
...
func (m *Base) AfterSaveCommit() {
	//fmt.Println("!!!!!!!!!!!!!!!!!!!! AfterSaveCommit")
	//TODO: 暂不知道为什么数据更新会有延迟，所以先用这种方式来处理
	if len(m.afterSaveCallback) > 0 {
		go func() {
			l, _ := afterSaveCommit.LoadOrStore(m.InstanceID(), &sync.RWMutex{})
			l.(*sync.RWMutex).Lock()
			defer func() {
				l.(*sync.RWMutex).Unlock()
				afterSaveCommit.Delete(m.InstanceID())
			}()
