...
var (
	...
	RequestErr         = add(http.StatusBadRequest)          // 请求错误
	Unauthorized       = add(http.StatusUnauthorized)        // 未认证
	AccessDenied       = add(http.StatusForbidden)           // 访问权限不足
	NothingFound       = add(http.StatusNotFound)            // 啥都木有
	MethodNotAllowed   = add(http.StatusMethodNotAllowed)    // 不支持该方法
	Conflict           = add(http.StatusConflict)            // 冲突
	ServerErr          = add(http.StatusInternalServerError) // 服务器错误
	ServiceUnavailable = add(http.StatusServiceUnavailable)  // 过载保护,服务暂不可用
	Deadline           = add(http.StatusGatewayTimeout)      // 服务调用超时
	NotImplemented     = add(http.StatusNotImplemented)      // 接口未实现
